...
def calc_퇴직소득세(근속년수: int, 이연퇴직소득: int) -> Dict[str, float]:
    ...
    퇴직소득세율 = float(퇴직소득산출세액) / float(이연퇴직소득) if 이연퇴직소득 > 0 else 0.0
    지방소득세율 = 0.10
...
def calc_연금수령가능일(
    ...
    - C25 = MAX(55세되는날, IRP가입일, 퇴직일)
    - C26 = MAX(55세되는날, IRP가입일+5년)
    - 퇴직일이 없으면(재직 중) 연금수령가능일 = C26
      있으면 연금수령가능일 = MAX(퇴직일, MIN(C25, C26))
    """
    ...
    # C25 = MAX(55세, IRP가입일, 퇴직일[옵션])
    candidates = [생55세, IRP가입일_dt]
    if 퇴직일_dt is not None:
        candidates.append(퇴직일_dt)
    C25 = max(candidates)

    # C26 = MAX(55세, IRP가입일+5년)  (윤년 보정)
    try:
        IRP가입일_5년후 = IRP가입일_dt.replace(year=IRP가입일_dt.year + 5)
    except ValueError:
        IRP가입일_5년후 = IRP가입일_dt.replace(year=IRP가입일_dt.year + 5, month=2, day=28)
    C26 = max(생55세, IRP가입일_5년후)

    # 연금수령가능일
    if 퇴직일_dt is None:
        연금수령가능일 = C26
    else:
        연금수령가능일 = max(퇴직일_dt, min(C25, C26))
...
def simulate_pension(
    ...
    # 개시일 '전'에 찍을 첫 행만 필요하다면 조건을 더 좁혀도 됩니다.
        rows.append({
            ...
            "한도기준잔액(1/1)": 0,
            "한도": None, "한도초과분": 0,
            "초과_과세제외": 0, "초과_이연퇴직": 0, "초과_그외": 0,
            "이내_과세제외": 0, "이내_이연퇴직": 0, "이내_그외": 0,
            "초과_과세제외_소득세": 0, "초과_이연퇴직_소득세": 0, "초과_그외_소득세": 0,
            "이내_과세제외_소득세": 0, "이내_이연퇴직_소득세": 0, "이내_그외_소득세": 0,
            "소득세_합계": 0, "초과_과세제외_지방세": 0, "초과_이연퇴직_지방세": 0, "초과_그외_지방세": 0,
            "이내_과세제외_지방세": 0, "이내_이연퇴직_지방세": 0, "이내_그외_지방세": 0, "지방세_합계": 0,
            "총세액": 0, "실수령액": 0, "세율": 0,
            "지급옵션": "개시전",
        })
    ...
    if 연금지급일 >= 연금개시일_dt:
        연금지급일 = 연금개시일_dt   # 🔧 개시년도는 개시일 당일로
          # 검증용(결과 미사용)
        U = 연금수령연차 + max(0, 연금개시일_dt.year - 연금수령가능일.year)                   # 연차 1로 시작
        지급회차 = 1
    ...
    while True:        
        ...
        if U == 연금수령연차:
            ...
            year_base = 과세기간개시일.year if 연금지급일.year == 과세기간개시일.year else 연금지급일.year
        ...
        # 총 지급/잔액
        실제지급액 = (이내_과세제외 + 이내_이연퇴직 + 이내_그외 +
                    초과_과세제외 + 초과_이연퇴직 + 초과_그외)
        지급후적립금 = 적립금 - 실제지급액
        운용수익 = 지급후적립금 * 운용수익률
        ...
        소득세_합계 = (초과_과세제외_소득세 + 초과_이연퇴직_소득세 + 초과_그외_소득세 +
                    이내_과세제외_소득세 + 이내_이연퇴직_소득세 + 이내_그외_소득세)
        지방세_합계 = (초과_과세제외_지방세 + 초과_이연퇴직_지방세 + 초과_그외_지방세 +
                    이내_과세제외_지방세 + 이내_이연퇴직_지방세 + 이내_그외_지방세)
        총세액 = 소득세_합계 + 지방세_합계
    ...
    df_capped["과세기간개시일"] = pd.to_datetime(df_capped["과세기간개시일"], errors="coerce")    
