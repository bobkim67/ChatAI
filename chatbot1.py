...
with tab2:
    ...
    # 금액 관련
    과세제외_자기부담금 = int(_sel_row.get("그외과세제외금액", 0))
    이연퇴직소득 = int(_sel_row.get("퇴직소득금액", 0)) + int(
        _sel_row.get("사용자부담금", 0)
    )
    세액공제자기부담금 = int(_sel_row.get("소득공제금액", 0))
    운용손익 = (
        int(_sel_row.get("평가금액", 0))
        - 과세제외_자기부담금
        - 이연퇴직소득
        - 세액공제자기부담금
    )
    운용수익률 = 0.03
    이연퇴직소득세 = int(_sel_row.get("이연퇴직소득세", 0))
    ...
    if 이연퇴직소득세 is not None and 이연퇴직소득세 > 0:
        ...
        _calc = pnsn_sim_calculator.calc_퇴직소득세(
